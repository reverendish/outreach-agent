          ...
          <div style={{ display: "grid", gap: 20 }}>
            ...
            <div className="card" style={{ padding: 0, overflow: "hidden" }}>
              <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h2 style={{ fontSize: "0.9rem", fontWeight: 700, color: "var(--text)" }}>
                  Follow-up queue
                  {followUpQueue.length > 0 && <span className="badge badge-amber" style={{ marginLeft: 8 }}>{followUpQueue.length}</span>}
                </h2>
              </div>
              {followUpQueue.length === 0 ? (
                <p style={{ padding: 24, fontSize: "0.875rem", color: "var(--faint)", textAlign: "center" }}>Nothing overdue. 🎉</p>
              ) : (
                <div>
                  {followUpQueue.slice(0, 8).map(c => {
                    const overdueDays = c.sequenceState?.nextSendDue ? daysSince(c.sequenceState.nextSendDue) : 0;
                    return (
                      <div key={c.id} onClick={() => router.push(`/contact/${c.id}`)}
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          ...
                          <div>
                            <p style={{ fontSize: "0.875rem", fontWeight: 600, color: "var(--text)" }}>{toTitleCase(c.ch?.companyName ?? "Unknown")}</p>
                            <p style={{ fontSize: "0.75rem", color: "var(--muted)" }}>Step {c.sequenceState?.currentStepNumber}</p>
                          </div>
                        </div>
                        <span className="badge badge-red">{overdueDays === 0 ? "Due today" : `${overdueDays}d overdue`}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="card" style={{ padding: 0, overflow: "hidden" }}>
              ...
              {contacts.length === 0 ? (
                ...
                [...contacts].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 5).map(c => (
                  <div key={c.id} onClick={() => router.push(`/contact/${c.id}`)}
                    style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 20px", borderBottom: "1px solid var(--border)", cursor: "pointer" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      {c.starred && <span style={{ color: "var(--accent)", fontSize: "0.85rem" }}>★</span>}
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <span style={{ fontSize: "0.72rem", color: "var(--faint)" }}>{daysSince(c.createdAt) === 0 ? "Today" : `${daysSince(c.createdAt)}d ago`}</span>
                      <span className={`badge ${c.status === "converted" ? "badge-green" : c.status === "contacted" || c.status === "replied" ? "badge-amber" : "badge-blue"}`}>
                ...
                return (
                  <div key={stage}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                      <span style={{ fontSize: "0.78rem", fontWeight: 700, color: "var(--muted)", textTransform: "capitalize" }}>{PIPELINE_STAGE_LABELS[stage]}</span>
                    ...
                    <div style={{ display: "grid", gap: 6 }}>
                      {stageContacts.slice(0, 12).map(c => (
                        <div key={c.id} onClick={() => router.push(`/contact/${c.id}`)}
                          ...
                          <p style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text)", marginBottom: 2 }}>{toTitleCase((c.ch?.companyName ?? "Unknown").slice(0, 28))}</p>
                          <p style={{ fontSize: "0.7rem", color: "var(--faint)" }}>{daysSince(c.updatedAt)}d in stage</p>
                        </div>
                      ))}
                      {stageContacts.length > 12 && <p style={{ fontSize: "0.72rem", color: "var(--faint)", textAlign: "center", padding: "6px 0" }}>+{stageContacts.length - 12} more</p>}
                    </div>
                  </div>
                );
              })}
